// ===================================================================
// === ALFA SAT: The Complete and Corrected editor.js File (v3)    ===
// ===================================================================
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let savedQuestions = {};
    let editors = { passage: null, prompt: null, options: {}, fillIn: null };
    ...
    const questionTypes = {
        ...
        "Math": { "Algebra": ["Linear equations in one variable", "Linear functions"], "Advanced Math": ["Equivalent expressions", "Nonlinear functions"] }
    };
    const pointValues = [10, 20, 30, 40];

    // --- Initial Setup ---
    if (!testId) { window.location.href = 'admin.html'; return; }
    ...
    function cleanupStimulusPanel() {
        ...
        editors = { passage: null, prompt: null, options: {}, fillIn: null };
    }
    ...
    async function uploadImageToTelegram(file) {
        ...
        try {
            ...
            if (data.ok) {
                ...
                if (fileUrlData.ok) { return `https://api.telegram.org/file/bot${TELEGRAM_BOT_TOKEN}/${fileUrlData.result.file_path}`; }
                else { throw new Error(fileUrlData.description); }
            } else { throw new Error(data.description); }
        } catch (error) { console.error('Telegram Upload Error:', error); alert('Error uploading image: ' + error.message); return null; }
    }
    ...
    async function showEditorForQuestion(module, qNumber) {
        ...
        const quillToolbarOptions = [['bold', 'italic', 'underline'],[{ 'list': 'ordered'}, { 'list': 'bullet' }],[{ 'align': [] }],['formula'],['clean']];
        const quillOptions = { modules: { toolbar: quillToolbarOptions }, placeholder: 'Type or paste content here...', theme: 'snow' };
    ...
    function handleNavClick(e) { showEditorForQuestion(currentModule, e.target.dataset.qNumber); }
